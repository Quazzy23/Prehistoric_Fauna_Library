...
def update_info_file(genus, full_name, artist, action, stage=None, note=None, base_spec=None, scale_spec=None, target_status=None, target_user=None, stage_author=None):
    """Универсальный записыватель info.txt. Возвращает (status, user) для синхронизации JSON."""
    info_path = os.path.join(MODELS_ROOT, genus, full_name, f"{full_name} info.txt")
    if not os.path.exists(info_path): return target_status, target_user
    
    try:
        with open(info_path, 'r', encoding='utf-8') as f: lines = f.readlines()
        with open(info_path, 'w', encoding='utf-8') as f:
            for line in lines:
                if line.startswith("model_status:") and target_status:
                    f.write(f"model_status: {target_status}\n")
                elif line.startswith("claimed_by:") and target_user is not None:
                    f.write(f"claimed_by: {target_user}\n")
                elif line.startswith("base_specimen:") and base_spec:
                    f.write(f"base_specimen: {base_spec}\n")
                elif line.startswith("scale_specimen:") and scale_spec:
                    f.write(f"scale_specimen: {scale_spec}\n")
                elif stage and line.startswith(f"{stage}:") and stage_author:
                    f.write(f"{stage}: {stage_author}\n")
                    ...
                    f.write(f"{ts} [{artist}] [{action.upper()}]: {note}\n")
                else:
                    f.write(line)
        return target_status, target_user
...
def review_mode(catalog, user):
    ...
    while True:
        ...
        if target in ['-', 'back', '*', 's', '!', 'my', 'release', 'submit', 'review']: 
            if target in ['-', 'back']: print()
            return target
        ...
        st_name = match['stage'].upper()
        ans = get_input(f"Approve {st_name} for '{display_name}'? (y/n) > ")
        
        if ans == 'y':
            user_note = get_optional_note()
            new_stat = "finished" if match['stage'] == "rig" else "free"
            res_status, res_user = update_info_file(match['genus'], display_name, user, 'approve', match['stage'], user_note, target_status=new_stat, target_user="", stage_author=match['user'])
            if res_status:
                stages_flow = ["skeletal", "mesh", "texture", "rig", "finished"]
                try:
                    next_idx = stages_flow.index(match['stage']) + 1
                    match['stage'] = stages_flow[next_idx]
                except: pass
                match['m_status'], match['user'] = res_status, res_user
                match['s_author'] = "" if new_stat == "free" else match['user']
                save_catalog_silently(catalog)
                print_species_card(match)
                print(f"\n[+] {st_name} approved. Asset is now {new_stat.upper()}.\n")
                if new_stat == "finished": print("THE MODEL IS READY! HOORAY! REX✨\n")
                return 
        elif ans == 'n':
            while True:
                reason = input("Enter reason for rejection (note) > ").strip()
                if not reason:
                    if get_input("Are you sure you want to set NEEDS_FIX status without comments? (y/n) > ") == 'y':
                        note_to_save = None; break
                    else: continue
                else: note_to_save = reason; break
            
            res_status, res_user = update_info_file(match['genus'], display_name, user, 'reject', match['stage'], note_to_save, target_status="needs_fix", target_user=match['user'])
            if res_status:
                match['m_status'], match['user'] = res_status, res_user
                save_catalog_silently(catalog)
                print_species_card(match)
                print(f"\n[!] Status changed to NEEDS_FIX. {'Note added.' if note_to_save else 'No notes added.'}\n")
                return
        else: print("[-] Action cancelled.\n"); return
...
def release_my_claims(catalog, user):
    ...
    print("\nYOUR ACTIVE CLAIMS:")
    for item in my_tasks: print_species_card(item); print()
    ...
    while True:
        ...
        if target in ['-', 'back', '*', 's', '!', 'my', 'release', 'submit', 'review']: 
            ...
            return target
        if target == '*': 
            for item in my_tasks: print_species_card(item); print(); continue
        ...
        if get_input(f"Release '{display_name}'? (y/n) > ") == 'y':
            user_note = get_optional_note()
            new_stat = "needs_fix" if match.get('s_author') else "free"
            new_user = match['s_author'] if new_stat == "needs_fix" else ""
            res_status, res_user = update_info_file(match['genus'], display_name, user, 'release', match['stage'], user_note, target_status=new_stat, target_user=new_user)
            if res_status:
                match['m_status'], match['user'] = res_status, res_user
                save_catalog_silently(catalog)
                print_species_card(match)
...
def submit_my_work(catalog, user):
    ...
    while True:
        ...
        display_name = format_sp_name(match)
        st_name = match['stage'].upper()
        if get_input(f"Submit {st_name} for '{display_name}' for review? (y/n) > ") == 'y':
            b_s, s_s = (None, None)
            if st_name == "SKELETAL":
                print("\nSCIENTIFIC DATA ENTRY:")
                b_s = input("Enter Base Specimen (Anatomy source) > ").strip()
                s_s = input("Enter Scale Specimen (Size source) > ").strip()
            user_note = get_optional_note()
            res_status, res_user = update_info_file(match['genus'], display_name, user, 'submit', match['stage'], user_note, b_s, s_s, target_status="review", target_user=user, stage_author=user)
            if res_status:
                match['m_status'], match['user'] = res_status, res_user
                match['s_author'] = user
                save_catalog_silently(catalog)
...
def main():
    user = local_settings.USER_NAME
    catalog = load_catalog()
    print(f"\n>>> PFL ASSET MANAGER \t Hello, {user}! :)")
    print(f"0 / exit \n- / back \n* / release \ns / submit \n! / review")
    print(f"all [status] [stage] (e.g. all free mesh)\n")
    
    last_results = [] 
    jump = None

    while True:
        if jump: action = jump; jump = None
        else:
            prompt = "Enter command or genus name > " if not last_results else "Enter command, genus or species name > "
            action = get_input(prompt)

        if action in ['-', 'back']: last_results = []; continue 
        if action in ['*', 'release', 'my']: jump = release_my_claims(catalog, user); last_results = []; continue
        if action in ['s', 'submit']: jump = submit_my_work(catalog, user); last_results = []; continue
        if action in ['!', 'review']: jump = review_mode(catalog, user); last_results = []; continue

        if action == "all" or action.startswith("all "):
            parts = action.split()
            valid_filters = ["free", "busy", "review", "finished", "needs_fix", "skeletal", "mesh", "texture", "rig"]
            if not all(p in valid_filters for p in parts[1:]): print(f"[-] No results for '{action}'\n"); continue
            ...
            if not filtered_list: print(f"[-] No species match these filters: {action}\n")
            else:
                for item in filtered_list: print_species_card(item)
                print(); last_results = filtered_list 
            continue

        species_match = None
        if last_results: species_match = next((s for s in last_results if s['species'].lower() == action), None)
        ...
        if species_match:
            ...
            if m_status in ['free', 'needs_fix']:
                if m_status == 'needs_fix' and species_match['user'] and user.lower() != species_match['user'].lower():
                    print(f"\n[!] WARNING: This asset was previously worked on by @{species_match['user']}.")
                    if get_input("Are you sure you want to take it? (y/n) > ") != 'y': print("[-] Claim cancelled.\n"); continue

                current_stage = species_match['stage'].upper()
                if get_input(f"Claim {current_stage} for '{display_name}'? (y/n) > ") == 'y':
                    user_note = get_optional_note()
                    res_status, res_user = update_info_file(species_match['genus'], display_name, user, 'claim', species_match['stage'], user_note, target_status="busy", target_user=user)
                    if res_status:
                        species_match['m_status'], species_match['user'] = res_status, res_user
                        save_catalog_silently(catalog)
